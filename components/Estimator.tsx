...
// Keys for ETC category
const ETC_ITEMS = [
  { id: 'point_wrap', basePrice: 400000 },
  { id: 'sanding_add', basePrice: 100000 },
  { id: 'windshield', basePrice: 600000 },
  { id: 'remove_wrap', basePrice: 300000 },
  { id: 'door_ppf', basePrice: 400000 },
  { id: 'chrome_delete', basePrice: 700000 },
  { id: 'door_chrome_delete', basePrice: 500000 },
  { id: 'front_ppf_full', basePrice: 1700000 },
  { id: 'sidemirror', basePrice: 200000 },
  { id: 'light_ppf', basePrice: 200000 },
  { id: 'door_jam', basePrice: 500000 },
  { id: 'indoor_ppf', basePrice: 300000 },
  { id: 'b_pillar', basePrice: 150000 },
  { id: 'headlight_smog', basePrice: 250000 },
];

const REGION_KEYS = [
  'gangnam', 'songpa', 'yongsan', 'yeongdeungpo', 'gangbuk',
  'bundang', 'anyang', 'ilsan', 'incheon', 'daejeon',
  'daegu', 'busan', 'gwangju', 'gangwon'
];
...
export const Estimator: React.FC<EstimatorProps> = ({ isOpen, onClose }) => {
  ...
  const handleConsultation = useCallback(async () => {
    ...
    if (coverage === CoverageType.ETC) {
      ...
      setAiAdvice(t('estimator.labels.ai_advice_default'));
      setLoading(false);
      return;
    }
  ...
  const getSelectedPrice = () => {
    if (coverage === CoverageType.ETC) {
      return selectedEtcIndex !== null ? ETC_ITEMS[selectedEtcIndex].basePrice : 0;
    }
          ...
          <div className="overflow-y-auto p-8 md:p-12 lg:p-16">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              ...
              <div className="space-y-12">
                <div>
                  <h3 className="text-2xl font-serif text-white mb-2 word-keep-all">{t('estimator.title')}</h3>
                  <p className="text-neutral-500 text-sm word-keep-all">{t('estimator.subtitle')}</p>
                </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className={`group ${coverage === CoverageType.ETC ? 'opacity-50 pointer-events-none' : ''}`}>
                    <label className="block text-xs uppercase tracking-widest text-gold-400 mb-3 cursor-pointer">
                      {t('estimator.step2')} {size ? `(${size.split('(')[1]?.replace(')', '') || size})` : ''}
                    </label>
                    <div
                    ...
                    >
                      <span className={`text-lg font-serif ${carModel ? 'text-white' : 'text-neutral-700'}`}>
                        {carModel || (vehicleList.length > 0 ? t('estimator.select_model') : t('estimator.no_data'))}
                      </span>
                  ...
                  <div>
                    ...
                    {coverage === CoverageType.FRONT_PACKAGE && (
                      <p className="text-xs text-gold-400 mt-2 px-2">
                        {t('estimator.front_package_notice')}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="pt-6">
                    ...
                    >
                      ...
                      {loading ? t('estimator.btn_analyzing') : (coverage === CoverageType.ETC ? t('estimator.btn_etc') : t('estimator.btn_analyze'))}
                    </Button>
                  </div>
                </div>
              </div>
              ...
              <div className="relative border-l border-neutral-800 pl-0 lg:pl-16 flex flex-col justify-center min-h-[500px]">
                {!estimates && !loading && (
                  <div className="text-neutral-700 text-center lg:text-left">
                    <p className="font-serif italic text-2xl mb-4 word-keep-all">{t('estimator.result_title')}</p>
                    <p className="text-sm tracking-widest uppercase">{t('estimator.result_subtitle')}</p>
                  </div>
                )}
                ...
                {loading && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-gold-400">
                    ...
                    <p className="text-xs tracking-widest uppercase animate-pulse">{t('estimator.consulting')}</p>
                  </div>
                )}
                  ...
                  <div className="animate-fade-in space-y-10">
                    ...
                    {coverage === CoverageType.ETC ? (
                      <div className="border-b border-neutral-800 pb-6 relative group/list">
                        <p className="text-xs text-gold-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                          ...
                          {t('estimator.labels.other_prices')}
                        </p>
                        ...
                        <div className="relative">
                            ...
                            {ETC_ITEMS.map((item, index) => (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  ...
                                  <span className={`font-medium ${selectedEtcIndex === index ? 'text-white' : 'text-neutral-300'}`}>
                                    {t(`estimator.options.${item.id}`)}
                                  </span>
                                </div>
                                <span className={`font-serif ${selectedEtcIndex === index ? 'text-gold-400' : 'text-white'}`}>
                                  {t(`estimator.options.${item.id}_price`)}
                                </span>
                              </button>
                            ))}
                          </div>
                          ...
                          <div className={`absolute bottom-6 left-1/2 -translate-x-1/2 transition-all duration-500 z-20 ${showScrollHint ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                            <div className="bg-gold-500 text-black px-4 py-2 rounded-full shadow-[0_0_15px_rgba(198,154,38,0.4)] flex items-center gap-2 animate-bounce">
                              <span className="text-xs font-bold whitespace-nowrap">{t('estimator.labels.scroll_more')}</span>
                        ...
                        <p className="text-neutral-500 text-xs mt-4">{t('estimator.specific_matte_notice')}</p>
                      </div>
                    ) : (
                      /* Standard & Front Package View */
                      <div className="border-b border-neutral-800 pb-6">
                        <p className="text-xs text-gold-400 uppercase tracking-widest mb-4">{t('estimator.estimate_title')}</p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <span className={`text-xs font-bold uppercase tracking-wider ${selectedPackage === 'transparent' ? 'text-white' : 'text-neutral-300'}`}>{t('estimator.labels.ppf_clear')}</span>
                            </div>
                            ...
                            {coverage === CoverageType.FRONT_PACKAGE && (
                              <p className="text-[10px] text-neutral-500 mt-1">{t('estimator.front_package_special')}</p>
                            )}
                          </button>
                          ...
                          {coverage === CoverageType.FULL_BODY && (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className={`text-xs font-bold uppercase tracking-wider ${selectedPackage === 'matte' ? 'text-white' : 'text-neutral-300'}`}>{t('estimator.labels.ppf_matte')}</span>
                              </div>
                              <div className="text-xl font-serif text-white">
                          ...
                          {coverage === CoverageType.FULL_BODY && (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className={`text-xs font-bold uppercase tracking-wider ${selectedPackage === 'color' ? 'text-white' : 'text-neutral-300'}`}>{t('estimator.labels.ppf_color')}</span>
                              </div>
                              <div className="text-xl font-serif text-white">
                                {estimates.color.min > 0
                                  ? `₩${formatPrice(estimates.color.min)}~`
                                  : <span className="text-sm text-neutral-500">{t('estimator.ask_separately')}</span>}
                              </div>
                            </button>
                          )}
                          ...
                          {coverage === CoverageType.FULL_BODY && (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className={`text-xs font-bold uppercase tracking-wider ${selectedPackage === 'wrap' ? 'text-white' : 'text-neutral-300'}`}>{t('estimator.labels.wrap_premium')}</span>
                              </div>
                              <div className="text-xl font-serif text-white">
                        ...
                        <p className="text-neutral-500 text-xs mt-4">{t('estimator.final_quote_notice')}</p>
                      </div>
                    )}
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="text-sm font-medium text-white tracking-wide">{t('estimator.concierge_comment')}</h4>
                      </div>
                    ...
                    <div className="pt-4">
                      {!isRegionSelectMode ? (
                        ...
                        >
                          {t('estimator.btn_consultation')} <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                        </Button>
                      ) : (
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-2 gap-2">
                            {REGION_KEYS.map((key) => (
                              <button
                                ...
                                onClick={() => handleRegionClick(t(`regions.${key}`))}
                              >
                                {t(`regions.${key}`)}
          ...
          <div className="w-full max-w-2xl bg-neutral-900 border border-neutral-800 rounded-sm shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="p-6 border-b border-neutral-800 flex items-center gap-4">
              ...
              <input
                autoFocus
                type="text"
                placeholder={`${t('estimator.search_placeholder')} (${size.split('(')[0]})`}
            ...
            <div className="max-h-[60vh] overflow-y-auto">
              ...
              {searchQuery && !filteredCars.some(c => c.toLowerCase() === searchQuery.toLowerCase()) && (
                ...
                >
                  <span className="font-medium tracking-wide">{t('estimator.labels.manual_input')}: "{searchQuery}"</span>
                  <Check className="w-4 h-4" />
                </button>
              )}
